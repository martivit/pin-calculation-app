...
######################
    ...
    legend_labels = [
        ...
        'Face unacceptable learning conditions',
        'Live in an unprotected environment in relationship with education',
        "Face aggravating circumstances preventing their access to education, OoS"
    ]
    ...
    run_perc_in_need = paragraph_tot.add_run(f"{tot_perc_in_need}%")
    ...
    run_text = paragraph_tot.add_run(" of school-aged children are in need:")
    run_text.font.size = Pt(15)

    # Add a line break and then the number of children in need, making it bold
    run_text = paragraph_tot.add_run("\n" + format_number(tot_in_need))
    run_text.font.size = Pt(15)
    run_text.font.color.rgb = RGBColor(86,150,151)
    run_text.bold = True

    # Add the remaining text after the number of children in need (not bold)
    run_text = paragraph_tot.add_run(" children including:")
    run_text.font.size = Pt(15)
    ...
    bullet_text = bullet_point_girls_boys.add_run(f"{format_number(tot_in_need_girl)} girls and {format_number(tot_in_need_boy)} boys;")
    ...
    bullet_point_format.left_indent = Inches(1)
    bullet_text = bullet_point_disability.add_run(f"{format_number(tot_in_need_disability)} children with disability.")
    bullet_text.font.size = Pt(12)
    bullet_text.font.name = 'Calibri'
